...
namespace DocuSign.MyClickwrap.COVID19Waiver.Security
{
    ...
    {
        public static void ConfigureDocuSign(
        ...
        public static void ConfigureDocuSignExceptionHandling(
                    ...
                    if (contextFeature != null)
                    {
                        switch (contextFeature.Error)
                        {
                            ...
                            case AuthenticationException error:
                                ...
                                break;
                            default:
                                logger.LogError($"Error occured: {contextFeature.Error.Message}");
                                logger.LogError($"Stack Trace: {contextFeature.Error.StackTrace}");
                                break;
                        }
                        ...
                        if (context.Request.Path.StartsWithSegments("/api"))
                        {
                            await context.Response.WriteAsync(new ErrorDetails
                            {
                                ...
                                Message =
                                    "Internal server error occured. Please refer to the log file for the details."
                            }.ToString());
                        }
                        else
                        {
